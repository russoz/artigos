...
O L<DataFlow> irá transformar essas subrotinas em objetos L<DataFlow::Proc>,
...
mais de uma forma:
	...
	# or process everything in one single blow
	my @res = $uc->process( 'aa', 'bb', 'cc' );

Além disso o L<DataFlow>

O racional de um C<DataFlow::Node> é o mesmo de um comando de Unix ou Linux
que atua como um filtro. E a idéia de um C<DataFlow::Chain> seria a mesma de
um I<pipe> na linha de comando, e se usaria assim:
