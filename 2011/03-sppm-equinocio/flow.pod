...
Em termos de programação em C<Perl>, podemos colocar vários tipos de
porquinhos:
...
B<Atenção>: nesta última forma, os elementos do ArrayRef C<não> serão
...
Esse nó simplesmente pega o valor de C<$base_url>, uma URL (no caso vamos
presumir que seja a URL do B<Cadastro de Empresas Inidôneas ou Sancionadas>
do site L<PortalTransparência|http://www.portaltransparencia.gov.br>) e a
injeta na fila de entrada do nó. Como L<DataFlow::Node::LiteralData> não
modifica o valor, essa mesma URL será o único item de saída desse nó.
