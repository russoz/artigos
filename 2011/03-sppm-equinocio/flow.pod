...
Logo isso evoluiu para a idéia de produzir um framework para buscar e 
analisar dados públicos. 
Sem demora, o L<Thiago Rondon|http://www.maluco.com.br/blog/> montou um 
L<repositório no GitHub|https://github.com/maluco/OpenData-BR> e começou a 
rabiscar umas idéias de código, e logo em seguida o site do
L<OpenData-BR|http://www.opendatabr.org/>. O projeto
...
E como usar esse nó? Um nó pode ser usado de mais de um jeito:
...
L<DataFlow::Chain>, que pode ser usada assim:
...
Esse código irá imprimir na tela (C<STDERR>) algo como:
...
B<< <div id="listagemEmpresasSancionadas"> >>, e dentro do bloco delimitado
por essa tag, uma B<< <table> >>, dentro dela um B<< <tbody> >>, e dentro dele
todas as tags B<< <tr> >>, que naturalmente correspondem às linhas da tabela
...
o texto HTML de cada linha (B<< <tr> >>) encontrada nas tabelas de ambas as
...
de cada célula, ou seja, de cada tag B<< <td> >> que a linha (B<< <tr> >>)
contiver. No entanto, aqui passamos o parâmetro C<result_type> igual a
C<'VALUE'>, isso faz com que, por exemplo,
B<< <tr><td>1</td><td>aa</td></tr> >>, retorne B<< ( 1, 'aa' ) >>. Mas, como
...
B<< Thiago "Maluco" Rondon E<lt> thiago em aware ponto com ponto br E<gt> >>

Por tirar o OpenData-BR do plano das idéias e trazê-lo para a realidade.
