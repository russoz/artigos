...
L<git|http://git-scm.com>[2], I<software> originalmente escrito pelo próprio 
...
Vamos colar aqui o C<dist.ini> atual do projeto L<Queue::Base> e comentar
cada trecho.
...
Várias informações específicas do projeto.
Particularmente, o número da versão poderia também ser automatizado, e será
...
Meta-informações sobre o probjeto, como por exemplo o
endereço da I<homepage>, e/ou o repositório de controle de versões.
...
Usando a tag C<[@Basic]> incluímos todos aqueles I<plugins>
listados acima, mas também:

=over

=item * 
C<MetaJSON]>: Escolhemos gerar B<também> um arquivo C<META.json> com as
meta-informações do pacote para indexação no CPAN,

=item *
C<ReadmeFromPod]>: Geramos o arquivo C<README> automaticamente a partir do
POD do arquivo principal do módulo (no caso do Queue::Base, seria o arquivo
C<< lib/Queue/Base.pm >>, 

=item *
C<[InstallGuide]>: Geramos um arquivo C<INSTALL> com instruções de instalação
(de acordo com o I<builder> utilizado: MakeMaker, Module::Install ou
Module::Build)

=item *
C<[GitFmtChanges]>: Geramos o arquivo C<Changes> com a listagem das
modificações realizadas no projeto, a partir dos logs do B<git>.
...
Optamos por utilizar a tag C<[OurPkgVersion]> para inserir o número
...
utilizando o L<Pod::Weaver> para gerar trechos da documentação em formato POD.
...
C<< L<weaver.ini|https://github.com/russoz/DataFlow/blob/master/weaver.ini> >>
...
de algum módulo. Para esses e outros casos, podemos usar a tag C<[Prereqs]>,
...
Incluindo a tag C<[PerlTidy]> no seu C<dist.ini>, você terá a
...
L<Perl::Tidy> ou pode fornecer o seu próprio C<perltidyrc>.
...
L<Jerome Quelin|http://search.cpan.org/~jquelin/>, engloba os I<plugins>:
...
existem I<plugins> para o L<Subversion|http://subversion.apache.org/> e
o L<Mercurial|http://mercurial.selenic.com/>.
...
estilo, adicionando I<tags> no C<dist.ini>, como pode ser visto abaixo. Duas
...
O L<Dist::Zilla> é uma fantástica ferramenta de auxílio ao desenvolvedor.
Ela facilita o processo de desenvolvimento, testes, I<release>, e até mesmo
o de documentar seu código.
...
=item *
B<Ricardo Signes (RJBS)>, pelo Dist::Zilla e pela sua prestatividade. 
Quando comecei a converter o processo de I<build> do Queue::Base para
Dist::Zilla, entrei no canal 
#distzilla do irc.perl.org e comentei o fato. Ele foi um dos primeiros a
responder, e a se interessar.

=item *
B<Comunidade São Paulo Perl Mongers>, pelo companheirismo, pelas infinitas
risadas, pela dedicação com que todos zelam pela nossa linguagem de programação
predileta.
