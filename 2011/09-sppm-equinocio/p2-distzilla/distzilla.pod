...
Os nossos I<plugins> favoritos também podem ser vistos como uma coleção, e
por conseguinte podemos criar um I<bundle> com essa coleção. De fato,
L<vários autores fazem isso|http://search.cpan.org/search?query=Dist%3A%3AZilla%3A%3APluginBundle%3A%3A&mode=dist>.

=head2 Soltando o seu Bundle
...
Na hora de usar, podemos usar a opção C<signature>:

  [@Author::RUSSOZ]
  signature = 0

E o seu C<dist.ini> incluirá o plugin L<Dist::Zilla::Plugin::Signature>
somente se o valor de C<signature> for igual a 1.

A versão mais recente de @Author::RUSSOZ está
L<< disponível | https://github.com/russoz/Dist-Zilla-PluginBundle-Author-RUSSOZ/blob/master/lib/Dist/Zilla/PluginBundle/Author/RUSSOZ.pm >> no L<http://github.com>.
